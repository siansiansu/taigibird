---
title: 鷦眉科 tsiau-bâi-kho
description: 收錄鷦眉科 tsiau-bâi-kho ê 台語號名，做未來命名討論 ê 參考。
keywords: ["台語", "鳥類", "鳥仔", "台灣", "鳥名", "eBird", "鷦眉"]
---

這頁收錄鷦眉科 tsiau-bâi-kho ê 台語號名，◆ 表示建議用名，◇ 表示別名或保留用法，△ 表示通稱。

## 臺灣鷦眉 タイワンサザイチメドリ Taiwan Cupwing [[eBird](https://ebird.org/species/taiwrb1)]

![臺灣鷦眉 タイワンサザイチメドリ Taiwan Cupwing](https://inaturalist-open-data.s3.amazonaws.com/photos/89046451/medium.jpg)

(c) 洪廷維, some rights reserved (CC BY-NC)

**台語名**

- ◆ **臺灣鷦眉 tâi-uân-tsiau-bâi**

**號名理路**

- `臺灣鷦眉 tâi-uân-tsiau-bâi` 先保留做這个物種 ê 建議名，作為目前檔內整理後 ê 主推形式。
- 這條目前主要是承接檔內既有整理脈絡；若後續補著較完整 ê 本土來源，猶會使再收斂號名理路。

---

## 小鷦眉 ミソサザイチメドリ Pygmy Cupwing [[eBird](https://ebird.org/species/pywbab1)]

![小鷦眉 ミソサザイチメドリ Pygmy Cupwing](https://inaturalist-open-data.s3.amazonaws.com/photos/60434124/medium.jpeg)

(c) Oscar Ho, all rights reserved

**台語名**

- ◆ **小鷦眉 sió-tsiau-bâi**

**號名理路**

- `小鷦眉 sió-tsiau-bâi` 先保留做這个物種 ê 建議名，作為目前檔內整理後 ê 主推形式。
